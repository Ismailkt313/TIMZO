...
const updateAcount = async (req, res) => {
  try {
    ...
    if (!fullname || !email) {
      return res
        .status(400)
        .json({ success: false, messaeg: "full name and email requird" });
    }
    const user = await User.findById(userId);
    if (!user) {
      return res
        .status(400)
        .json({ success: false, message: "user not founfd" });
    }
    ...
    return res
      .status(400)
      .json({ success: false, message: "internal serval error" });
  }
};
...
const verificationotp = async (req, res) => {
  try {
    ...
    if (!otp || !sessionOtp || !newEmail) {
      return res
        .status(400)
        .json({ success: false, message: "OTP verification data missing" });
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Email and profile updated" });
  } catch (error) {
